...
contract LogxStaker is IERC20, ILogxStaker, ReentrancyGuard, Ownable2StepUpgradeable {
     ...
     */
    function _getAccruedRewards(bytes32 stakeId, uint256 timestamp) internal view returns (uint256) {
        ...
        uint256 duration = timestamp >= stakeDurationEndTimestamp ? (userStake.duration * 1 days) : timestamp - stakeDurationStartTimestamp;
    ...
    function getUnclaimedUserRewards(bytes32 _account, uint256 timestamp) external view returns (uint256) {
        bytes32[] memory userStakeIds = userIds[_account];
        uint256 accruedRewards;
        for(uint256 i=0; i < userStakeIds.length; ++i) {
            accruedRewards = accruedRewards + _getUnclaimedRewards(userStakeIds[i], timestamp);
        }
        return accruedRewards;
    }

    function _getUnclaimedRewards(bytes32 stakeId, uint256 timestamp) internal view returns(uint256) {
        Stake memory userStake = stakes[stakeId];
        uint256 stakeDurationEndTimestamp = userStake.startTime + (userStake.duration * 1 days);
        uint256 lastRewardDistributionTime = lastRewardsTime[stakeId];

        uint256 duration = timestamp - lastRewardDistributionTime;
        if(userStake.duration != 0 && timestamp >= stakeDurationEndTimestamp) {
    ...
    function getStakeIdRewards(bytes32 stakeId, uint256 timestamp) external view returns (uint256) {
        return _getAccruedRewards(stakeId, timestamp);
    }
     ...
     */
    function unstakeForAccount(bytes32 _account, address _receiver, bytes32 _stakeId, uint256 timestamp) external nonReentrant returns(uint256){
        _validateHandler();
        return _unstake(_account, _receiver, _stakeId, timestamp);
    }

    function _unstake(bytes32 _account, address _receiver, bytes32 stakeId, uint256 timestamp) private returns(uint256) {
        require(!isStakeActive(stakeId, timestamp), "Staking duration active");
        bytes32 accountForStakeId = getAccountForStakeId(stakeId);
        require(accountForStakeId == _account, "Invalid stakeId for account");

        _updateRewards(_account, stakeId, timestamp);
    ...
    function restakeForAccount(bytes32 _account, bytes32 _stakeId, uint256 _duration, uint256 timestamp) external nonReentrant {
        _validateHandler();
        _restake(_account, _stakeId, _duration, timestamp);
    }

    function _restake(bytes32 _account, bytes32 _stakeId, uint256 _duration, uint256 timestamp) private {
        bytes32 accountForStakeId = getAccountForStakeId(_stakeId);
        require(accountForStakeId == _account, "invalid _stakeId for _account");
        require(!isStakeActive(_stakeId, timestamp), "staking duration active");

        _updateRewards(_account, _stakeId, timestamp);
        _updateStake(_stakeId, _duration, timestamp);
    }
    ...
    function claimTokensForAccount(bytes32 _account, address _receiver, uint256 timestamp) external nonReentrant returns (uint256) {
        _validateHandler();
        return _claimTokens(_account, _receiver, timestamp);
    }

    function _claimTokens(bytes32 _account, address _receiver, uint256 timestamp) private returns (uint256) {    
        bytes32[] memory userStakeIds = userIds[_account];
        for(uint256 i=0; i < userStakeIds.length; ++i) {
            _updateRewards(_account, userStakeIds[i], timestamp);
        }
    ...
    function isStakeActive(bytes32 stakeId, uint256 timestamp) public view returns (bool) {
        Stake memory userStake = stakes[stakeId];
    ...
    function _updateStake(bytes32 _stakeId, uint256 _duration, uint256 timestamp) private {
        require(stakes[_stakeId].startTime != 0, "Stake does not exist");
        ...
        lastRewardsTime[_stakeId] = timestamp;
    }

    //Note - following is the most important function in the contract.
    function _updateRewards(bytes32 _account, bytes32 stakeId, uint256 timestamp) private {
        Stake memory userStake = stakes[stakeId];
